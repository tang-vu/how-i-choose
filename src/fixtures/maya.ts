...
export const mayaProfile: CommunicationProfile = {
  ...
  disclosures: [
    ...[
      ...
      "rule-acknowledge",
      "rule-silence",
    ].map((fieldId, index) => ({ id: `disclosure-rule-${index + 1}`, fieldKind: "rule" as const, fieldId, agentVisible: true })),
    ...[
      "signal-yes",
      "signal-no",
      "signal-amber",
      ...
      "signal-pause",
      "signal-red",
    ].map((fieldId, index) => ({ id: `disclosure-signal-${index + 1}`, fieldKind: "signal" as const, fieldId, agentVisible: true })),
    { id: "disclosure-context", fieldKind: "context", fieldId: contextId, agentVisible: true },
    { id: "disclosure-scenario", fieldKind: "scenario_summary", fieldId: "scenario-community-workshop", agentVisible: true },
  ],
