...
Mocked tests prove page integration but cannot prove ChatGPT discovery. On 2026-08-31, the owner used deployed build `d9985080b5ab` in ChatGPT's built-in browser: all eight tools were discovered and the real-agent path exercised readiness, stale revision recovery, idempotency, deterministic rejection and repair, signal acknowledgment, Pause, Stop, reporting, guide verification, and draft-only patch staging. That run exposed the former display-only Human-only toggle. The owner then rechecked fixed production build `236b6c4d9b87`: reset produced Human-only `ready · v1`; brief and start both returned `AGENT_ACCESS_DISABLED` with no changed IDs; the visible Agent rehearsal control enabled scoped access; brief then returned `OK`, 20/20 shared fields, valid next actions, and `ready · v2`; no application console errors were observed. Evidence is separated in `JUDGE_CHECKLIST.md`.
